...
**IDs:** every idea has a stable `IDEA-NNN` id (never reused, even if dismissed) so it can be referenced directly ("approve IDEA-003"). **next id: IDEA-083**
...
- **IDEA-082 · Rule-36 DUAL-DOMAIN verification pass for every general-purpose node (owner-reminded)** — each model gets TWO real-data verdicts: (a) trading data AND (b) a real NON-trading dataset of the TYPE its theory consumes — koopman/bsts/fuzzy/grey on a classic non-financial TS (sunspots, airline-passengers, Mackey-Glass), anfis on a UCI regression set, infogeom/jump_diffusion/percolation/sv on a non-financial signal/network, hrp/cvar on a non-financial covariance, diffusion_map already done (pendigits). Download the datasets (Rule 33) and append the second-domain result to each report; a verdict citing only trading data stays PROVISIONAL. **Status:** approved (owner reminder = go-ahead)
