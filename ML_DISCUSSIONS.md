...
### 2026-06-24 — IDEA-088: forecasting-aligned objective FIXES P1 (SHADOW → KEEP)
Owner approved IDEA-088. Replaced P1's masked-reconstruction pretext with next-observation prediction (the CPC family core): `SelfSupervisedEncoder(objective="forecast")` now DEFAULT — latent trained to predict the future standardised obs, not reconstruct the past. Per-channel standardisation; forecast head `z@Wp+bp` predicts obs h-ahead; reconstruction objective retained for ablation/oracle. Fixed the node bug (its `_predict` had called recon-only `reconstruction_r2`; now `self_score`). RESULT (same OOS vol probe): **emb(forecast,6d) R²=+0.75 vs PCA +0.69 vs raw +0.82 → beats PCA on 7/10** (reconstruction had been +0.02, 0/10). **Verdict KEEP** — a better-than-PCA nonlinear compressor; doesn't beat raw-all-8ch (expected). Theory held exactly: reconstruction≠prediction; forecasting-aligned pretext keeps predictive directions (why TS2Vec/PatchTST/CPC transfer). Oracle 5/5, bank suite 11/11 (no breakage). Honest caveats logged: dir-acc=1.0 is a vol-autocorr artifact; emb>PCA not yet permutation-tested (IDEA-089). No impasse (Rule 31 N/A). P1 (vision keystone) COMPLETE + earning.
